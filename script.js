...
function submitBooking(event) {
    ...
    const travelers =
        form.querySelector('input[name="travelers"]')?.value
        || inputs[3]?.value;

    const titleElement =
        document.getElementById("bookingTitle");

    const place = titleElement
        ? titleElement.innerText.replace("Book ", "").trim()
        : "LankaExplore Destination";
    ...
    const bookings = JSON.parse(
        localStorage.getItem("lankaBookings") || "[]"
    );
...
(function () {
    ...
    function addWhatsAppButton() {
        ...
        /* Show on the public website, not on admin/login pages */
        if (
        ...
        if (document.getElementById("lankaWhatsAppButton")) {
            return;
        }

        const button = document.createElement("a");

        button.id = "lankaWhatsAppButton";
        button.href =
            "https://wa.me/94742119333?text=Hello%20LankaExplore%2C%20I%20would%20like%20to%20know%20more%20about%20Sri%20Lanka%20travel.";
        button.target = "_blank";
        button.rel = "noopener noreferrer";
        button.setAttribute("aria-label", "Chat with LankaExplore on WhatsApp");
        ...
        style.textContent = `
            ...
            #lankaWhatsAppButton:hover {
                transform: translateY(-4px) scale(1.02);
                box-shadow: 0 16px 35px rgba(0,0,0,.45);
            }

            #lankaWhatsAppButton .wa-icon {
                width: 24px;
                height: 24px;
                display: grid;
                place-items: center;
    ...
    /* ---------- Login user registration helper ---------- */

    window.registerLankaUser = function (user) {

        const users = JSON.parse(
            localStorage.getItem("lankaUsers") || "[]"
        );
